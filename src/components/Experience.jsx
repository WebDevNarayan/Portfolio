...
const experiences = [
  ...
  },
  {
    ...
    role:        'Frontend Developer',
    description: 'Collaborated in a cross-functional team (designer, frontend, backend) to deliver production-ready web applications on deadline. Took full ownership of UI components and responsive design.',
  ...
  },
  {
    ...
    description: 'My first professional engineering role. Shipped real features on a production e-commerce platform, learning JavaScript, React, and the discipline of writing code that others depend on.',
...
const Experience = () => {
  ...
  return (
    <section id="experience" ref={ref} className="relative py-32 lg:py-40 px-6 lg:px-20 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <span className="text-gray-600 font-mono text-xs tracking-[0.25em] uppercase">Where I've Worked</span>
          ...
          <div className="flex flex-col gap-14">
            {experiences.map((exp, i) => (
              <motion.div
                ...
                transition={{ duration: 0.72, delay: 0.3 + i * 0.16, ease: [0.33, 1, 0.68, 1] }}
                className="lg:flex gap-12 items-start"
              >

                {/* Year column */}
                <div className="hidden lg:flex flex-col items-end gap-1.5 w-[88px] shrink-0 pt-1.5">
                  <span className="text-gray-600 font-mono text-[11px] tracking-wider">{exp.year}</span>
                  {/* Timeline node */}
                  ...
                  <span className="text-gray-700 font-mono text-[10px] text-right mt-1">{exp.duration}</span>
                </div>

                {/* Card */}
                <div className="flex-1 p-7 rounded-2xl border border-white/[0.06] bg-white/[0.015]
                  ...
                  <div className="lg:hidden flex items-center gap-2 mb-4">
                    <span className="text-[#ff6200] font-mono text-[10px]">{exp.year}</span>
                    <span className="text-gray-700 font-mono text-[10px]">·</span>
                    <span className="text-gray-600 font-mono text-[10px]">{exp.duration}</span>
                  </div>
                  ...
                  <div className="mb-5">
                    ...
                    <h3 className="text-xl font-bold text-white group-hover:text-[#ff6200] transition-colors duration-400"
                  ...
                  {exp.projects.length > 0 && (
                    <div className="flex items-center gap-2 mb-5 flex-wrap">
                      <span className="text-gray-700 text-[11px] font-mono">Projects:</span>
                      {exp.projects.map(p => (
                        <span key={p} className="text-gray-400 text-[12px] font-mono">{p}</span>
                      ))}
                    </div>
                  )}
